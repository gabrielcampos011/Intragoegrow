...
export default async function SignupPage({
  ...
  return (
    <div className="flex min-h-screen bg-black-gradient text-white flex-col relative items-center justify-center p-4">
      ...
      <div className="absolute top-10 right-10 w-32 h-32 rounded-full bg-gogrow-red opacity-20 blur-3xl"></div>
      <div className="absolute bottom-10 left-10 w-64 h-64 rounded-full bg-gogrow-red opacity-10 blur-3xl"></div>
      ...
      <div className="z-10 w-full max-w-md bg-gogrow-gray-dark border border-white/10 rounded-2xl shadow-2xl overflow-hidden p-8">
        ...
        <p className="text-gray-400 text-center text-sm mb-8">
          Registre-se para acessar os treinamentos
        </p>
        ...
        <form action={signup} className="space-y-5">
          ...
          {params?.error && (
            <div className="p-3 bg-red-500/10 border border-red-500/50 rounded-lg">
              <p className="text-sm text-red-500 text-center">
                {params.message || 'Erro ao criar conta. Verifique os dados.'}
              </p>
            </div>
          )}
          ...
          <button
            type="submit"
            className="w-full py-3 px-4 bg-gogrow-red hover:bg-gogrow-red-hover text-white font-bold rounded-lg shadow-lg hover:shadow-gogrow-red/20 transition-all mt-6 active:scale-[0.98]"
          >
            Cadastrar
          </button>
        </form>
        ...
        <div className="mt-6 text-center text-sm text-gray-400">
          Já tem uma conta? <Link href="/login" className="text-white hover:text-gogrow-red hover:underline font-bold transition-colors">Faça login</Link>
        </div>
      </div>
    </div>
  )
