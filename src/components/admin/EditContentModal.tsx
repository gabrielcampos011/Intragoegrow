...
export default function EditContentModal({
  ...
  content: {
    ...
    sector_id: string | null; cover_url?: string | null
    description?: string | null; due_date?: string | null; duration_minutes?: number | null
  }
  sectors: any[],
  onClose: () => void
}) {
  ...
  const [contentUrl, setContentUrl] = useState(content.url)
  const [uploadingContent, setUploadingContent] = useState(false)

  // Cover image
  const [coverInputUrl, setCoverInputUrl] = useState(content.cover_url || '')
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              required
              value={type}
              onChange={(e) => { setType(e.target.value); setContentMode('url') }}
              ...
              <div className="border-2 border-dashed rounded-lg p-5 text-center">
                {uploadingContent ? (
                  ...
                  <div className="flex flex-col items-center gap-2 text-green-600">
                    <p className="text-sm font-semibold">✓ Novo arquivo enviado com sucesso</p>
                    <button
                      type="button"
                  ...
                  <label className="cursor-pointer flex flex-col items-center gap-2 text-gray-400 hover:text-gogrow-red transition-colors">
                    ...
                    <p className="text-sm font-medium">
                      Clique para substituir {type === 'pdf' ? 'o PDF' : 'o vídeo'}
                    </p>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2">
              ...
              <input
                ...
                type="number"
                min={1}
                defaultValue={content.duration_minutes || ''}
                className="w-full px-4 py-2 border rounded-lg focus:ring-1 focus:ring-gogrow-red focus:border-gogrow-red outline-none"
                placeholder="Ex: 30"
              />
            </div>
